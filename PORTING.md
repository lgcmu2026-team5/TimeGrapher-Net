...
원본: `D:\TimeGrapher_Refactoring\` (Qt Widgets / CMake). 타겟: Windows에서 실행되는
Avalonia(net8.0) 앱. **이 문서의 공개 API는 고정 계약이다 — 모든 포팅 작업자는
여기 명시된 시그니처를 그대로 구현하고, 임의로 바꾸지 않는다.**
...
6. Windows 전용 타겟: LinuxAudio(ALSA)는 포팅하지 않는다. WPF 사용 금지.

## 솔루션 구조 / 파일 소유권

```
TimeGrapherNet.sln
src/TimeGrapher.Core/                  net8.0 classlib (+NAudio)
  ...
  AudioIo/AudioCaptureWorker.cs        [Agent D] (NAudio WaveInEvent)
...
src/TimeGrapher.App/                   Avalonia 11.3 + ScottPlot.Avalonia 5.0
  ...
  Views/MainWindow.axaml(.cs)          [Agent G] (현재 placeholder — 전체 교체)
...
### Agent D — Audio I/O (`namespace TimeGrapher.Core.AudioIo`)

원본: `src/WindowsAudio.cpp`(QAudioSource 경로 참고용), `src/PlaybackWorker.cpp`,
`src/WavStreamWriter.cpp`, `include/WaveHeader.h`. 라이브 캡처는 Qt 대신 **NAudio
`WaveInEvent`** 사용(IEEE float mono, 요청 샘플레이트; winmm이 변환). 볼륨은
소프트웨어 곱(0..1).
...
public sealed class AudioCaptureWorker : IDisposable   // TAudioWorker 대응
{
    public AudioCaptureWorker(MasterAudioBuffer buffer);
    public event Action? DataReady;          // 캡처 콜백 스레드에서 발생
    public void Start(int deviceNumber, int sampleRate, float volume);
    public void SetVolume(float volume);     // 0..1
    public void Stop();
    public static IReadOnlyList<string> EnumerateInputDevices();           // WaveInEvent 디바이스명
    public static IReadOnlyList<int> GetSupportedSampleRates(int deviceNumber); // 최대 5개, 48000 포함 권장
}
...
- `Rendering/GraphFrameRenderer.cs`: QCustomPlot → ScottPlot 5 (`AvaPlot.Plot`).
  - Scope: 프레임마다 append + `purgeHistory`(GraphHistoryInSeconds 윈도) 동일 구현.
    48 kHz 데이터이므로 **Scatter 재생성 대신 ScottPlot `SignalXY`/누적 리스트 교체** 등
    성능을 고려할 것. x축은 `frame.GraphTickEnd` 우측 정렬, 폭 `sampleRate / scopeScale`.
...
- `Rendering/PixelBufferBitmap.cs`: `PixelBuffer`(ARGB32) → `WriteableBitmap`
  ...
  변환 + `Image` 컨트롤 갱신 헬퍼.
- 라이브 모드 장치/샘플레이트 enumerate는 Agent D의 정적 메서드 사용.
