...
포팅 이후, 약신호/고노이즈 환경 검출력 개선을 위해 원본 V5.x 검출기 의미론에서 **의도적으로 이탈하는 옵트인 옵션**을 추가했다. 전부 기본 OFF이며, OFF일 때 파이프라인이 원본 포팅과 비트동일함을 3중 장치가 보장한다(변경 전 기록한 골든마스터 테스트, null vs all-off 옵션 패리티 테스트, Verify `--fidelity-check` CI 게이트).
