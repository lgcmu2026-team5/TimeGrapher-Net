...
## 옵트인 V5.x 의미론 이탈 — 검출 강건성 옵션

포팅 이후, 약신호/고노이즈 환경 검출력 개선을 위해 원본 V5.x 검출기 의미론에서 **의도적으로 이탈하는 옵션**을 추가했다. Core 수준에서는 전부 기본 OFF이며, OFF일 때 파이프라인이 원본 포팅과 비트동일함을 3중 장치가 보장한다(변경 전 기록한 골든마스터 테스트, null vs all-off 옵션 패리티 테스트, Verify `--fidelity-check` CI 게이트). 다만 **App 합성 루트는 적응 플로어 + 레짐 가드를 기본 활성**한다 — 악조건 A/B 측정에서 두 옵션은 어느 행에서도 회귀가 없었고, 원본 소스가 임시 제공본이라 충실도 제약이 해제됐기 때문이다(소유자 결정). PLL 이벤트 베토만 GUI 체크박스 뒤 옵트인으로 남는다(극한 지속 잡음에서 recall 회귀가 측정됨). 원본 비트동일 경로는 Core 기본값과 Verify baseline 팔이 계속 보존·검증한다.
...
이 옵션들은 동결 계약인 `TgTypes.cs`를 건드리지 않도록 `TgDetector` 생성자 오버로드(`TgDetectorOptions`)로 들어가며, 조합 효과는 Verify의 악조건 행 A/B(`--ab=baseline,robust`)로 측정·게이트된다. 알려진 한계도 같은 하니스에 정직하게 핀되어 있다: 무음 선행 후 부트스트랩 리셋 폭풍(leadin-quiet 행)은 미해결이고, 극한 잡음에서 PLL 베토는 역효과다(noisy-1 행 — 미래 형태 기반 분류기의 몫).
