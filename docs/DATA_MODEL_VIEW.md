...
| `TgDetectorOptions`, `BeatCandidate`, `BeatEventGateConfig` | `Core.Detection`, `Core.Detection.Scoring`, `Core.Analysis` | Opt-in robustness knobs (all defaults off = pinned baseline behavior), the candidate-event context handed to an `IBeatEventGate` (event, sync state, thresholds, PLL match verdict captured at emission time), and the engine-level gate configuration carrier |
