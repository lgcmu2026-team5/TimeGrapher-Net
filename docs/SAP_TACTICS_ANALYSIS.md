...
| **state resynchronization + degradation recovery (PLL-guided sensitivity)** | BPH/PLL lock 이후에는 sync tracker가 예측한 A phase 근처에서 시작한 burst에 한해 min-peak 기준을 낮춘다. 후보 생성 자체를 전역으로 느슨하게 만들지 않고, 이미 재동기화된 시간 기준을 약신호 accept 판단에 재사용해 큰소리→조용함 전환 뒤의 검출 공백을 줄인다. 측정: quiet-step 행의 사후 recall/precision 0.694/1.000 → 1.000/1.000, missed beats 35 → 0, sync loss 1 → 0 | `TgDetector.cs`, `Detector.cs`, `DetectorStressScenarioTests.cs`, `AdverseScenarios.cs` | ✓ |
