...
work requests** — 점진적 저하). 패스마다 백로그를 **비트 주기 단위**(PLL 추적
`MeasuredPeriodS`, 락 전에는 125 ms 기본)로 환산해 2비트 예산과 비교하고, 연속
16패스 초과 시 시각 비용이 싼 순서로 저하한다:
...
남은 검증: 디스플레이가 연결된 Pi 5에서 GUI를 실제로 띄워
`./TimeGrapher.App --analysis-log ~/tg-gui-latency.csv`로 라이브 마이크 또는
Playback 실행의 캡처→처리, 처리→표시, 캡처→표시 레이턴시 CSV를 수집하고
단계별 비용 추정을 수치로 대체하는 것(ARCHITECTURE_REVIEW_FIXES.md의 미완 항목과 동일).
...
| **record/monitor (레이턴시 증거)** | QA가 요구하는 캡처→처리→표시 레이턴시를 단일 Stopwatch 시계로 계측: `MasterAudioBuffer`가 쓰기마다 (sampleEnd, ticks) 256개 스탬프 링을 유지, `AnalysisWorker`가 프레임에 `CaptureTimestamp`/`ProcessingCompletedTimestamp`를 스탬핑, UI가 렌더 직후 표시 시각을 더해 구간별 평균/최악값을 집계(`LatencyStatsTracker`, 상태바 우측 표시). 누락 비트(`WatchMetrics.MissedBeats`)·싱크 손실은 **세션 누적 카운터**로 프레임에 실려 latest-wins 병합에도 보존. Pi 5 실측 CSV는 GUI 렌더 완료 프레임만 기록하는 `--analysis-log <csv>`로 남기며, 컬럼은 캡처→처리/처리→표시/캡처→표시 레이턴시, 각 평균/최악값, 누적 드롭 오디오 샘플 수, 누락 비트 수로 제한한다 | `MasterAudioBuffer.cs`, `LatencyStatsTracker.cs`, `AnalysisPerformanceLogger.cs` | ✓ |
