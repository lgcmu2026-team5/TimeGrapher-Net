...
- 분석 패스는 **4096샘플 청크**(48 kHz에서 85.3 ms 분량) 단위로 돌고
  (`AnalysisWorker.cs:36`), 패스 시작 시점의 스냅샷까지만 드레인하므로 한 패스가
  라이브 쓰기를 무한정 쫓지 않는다.
...
| UI (활성 탭 1개) | 33/100 ms 스로틀 | 8000/250 포인트 예산, latest-wins 합류, 마커 plottable 풀링 |
...
1. 진행 중 사운드프린트 컬럼의 실시간 갱신 중단
2. 사운드프린트 발행 간격 100 ms → 400 ms
3. 스코프 데시메이션 stride 2배
...
| **maintain multiple copies of data** | 30초 링버퍼로 읽기/쓰기 속도 분리 + 사운드프린트 발행은 **고정 3버퍼 풀을 로테이션**하는 스냅샷 복사(발행된 버퍼는 2회의 더 새로운 발행 이후에만 재사용 → UI가 안전하게 읽는 동안 분석 스레드는 계속 갱신, 정상 상태 할당 0). 비트 노이즈 세그먼트 발행도 같은 패턴: **고정 16버퍼 풀(float[1600])을 로테이션**해 비트당 윈도를 발행하고, 발행된 세그먼트는 풀이 한 바퀴 돌기 전까지 불변 계약 — UI는 최신 스냅샷의 세그먼트만 읽고 캐시하지 않는다 | `MasterAudioBuffer`, `SoundPrintFrameProjector.cs`, `BeatSegmentCapture.cs` | ✓ |
